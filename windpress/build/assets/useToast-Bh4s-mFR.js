import { J as e, tt as t, vn as n } from "./runtime-core.esm-bundler-Twwq1XF8.js";
import { o as r } from "./base-CvBTaoCY.js";
...
function a() {
  ...
  function u(e2, t2) {
    a2.value[e2] = { ...a2.value[e2], ...t2, _duplicate: (a2.value[e2]._duplicate || 0) + 1 };
  }
  async function d() {
    if (!(s.value || c.length === 0)) {
      for (s.value = true; c.length > 0; ) {
        await t();
        let e2 = c.shift(), n2 = o?.value ?? 5;
        if (n2 <= 0) {
          a2.value.length && (a2.value = []);
          continue;
        }
        let r2 = a2.value.findIndex((t2) => t2.id === e2.id);
        if (r2 !== -1) {
          u(r2, e2);
          continue;
        }
        a2.value = [...a2.value, e2].slice(-n2);
      }
      s.value = false;
    }
  }
  function f(e2) {
    ...
    return n2 === -1 ? (c.push(t2), d(), t2) : (u(n2, t2), t2);
  }
  function p(e2, n2) {
  ...
  return { toasts: a2, add: f, update: p, remove: m, clear: h };
}
export {
  a as n,
  i as t
