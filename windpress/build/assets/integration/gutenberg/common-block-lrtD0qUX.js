...
import { n as e } from "../../index.browser-BEODCD8T.js";
import { t } from "../../isEqual-B9AJyoCE.js";
import { t as n } from "../../debounce-BMzMU_Rn.js";
import { t as r } from "../../preload-helper-CwQZjUKM.js";
import { t as i } from "../../windpress-D9qoLY9s.js";
import { t as a } from "../../useDarkMode-751yatJ3.js";
import { p as o } from "../../toggleHighContrast-CMSNQh1Z.js";
import { n as s, t as c } from "../../editor.worker-DPX7n8X5.js";
(async () => {
  function l({ attributes: e2 = {}, onChange: t2 }) {
    let [r2, i2] = wp.element.useState(``), [a2, o2] = wp.element.useState(``), s2 = Object.entries(e2).filter(([e3]) => e3 !== `class`), c2 = () => {
      ...
      let n2 = r2.trim().replace(/[^a-zA-Z0-9\-_:@]/g, ``);
      if (!n2) {
      ...
      delete r3[n2], t2(r3);
    }, u2 = wp.element.useRef(t2), d2 = wp.element.useRef(e2);
    wp.element.useEffect(() => {
      u2.current = t2, d2.current = e2;
    }, [
      t2,
      e2
    ]);
    let f2 = wp.element.useMemo(() => n((e3, t3) => {
      u2.current({
        ...d2.current,
        [e3]: t3
      });
    ...
    let p2 = wp.element.useCallback((e3, t3) => {
      f2(e3, t3);
    }, [
      f2
    ]);
    ...
    }, s2.map(([e3, t3]) => React.createElement(wp.components.Flex, {
      key: e3,
      gap: 2,
      style: {
        marginBottom: `8px`
  ...
  function f({ attributes: e2 = {}, onChange: t2 }) {
    let { src: r2 = ``, alt: i2 = ``, width: a2 = ``, height: o2 = `` } = e2, s2 = wp.element.useRef(t2), c2 = wp.element.useRef(e2);
    wp.element.useEffect(() => {
      s2.current = t2, c2.current = e2;
    }, [
      t2,
      e2
    ]);
    ...
    let d2 = wp.element.useCallback((e3, t3) => {
      u2(e3, t3);
    }, [
      u2
    ]);
  ...
  function h({ attributes: e2 = {}, onChange: t2 }) {
    let { href: r2 = ``, target: i2 = ``, rel: a2 = `` } = e2, [o2, s2] = wp.element.useState(false), c2 = wp.element.useRef(t2), l2 = wp.element.useRef(e2);
    wp.element.useEffect(() => {
      c2.current = t2, l2.current = e2;
    }, [
      t2,
      e2
    ]);
    let u2 = wp.element.useMemo(() => n((e3, t3) => {
      c2.current({
        ...l2.current,
        [e3]: t3
      });
    ...
    let d2 = wp.element.useCallback((e3, t3) => {
      ...
      e3?.url && (t3.href = e3.url, e3.opensInNewTab ? (t3.target = `_blank`, t3.rel = `noopener noreferrer`) : (delete t3.target, delete t3.rel)), c2.current(t3), s2(false);
    ...
    return React.createElement(wp.components.PanelBody, {
      ...
      checked: i2 === `_blank`,
      onChange: (n2) => {
        if (n2) d2(`target`, `_blank`), d2(`rel`, `noopener noreferrer`);
        else {
          let n3 = {
            ...e2
          };
          delete n3.target, delete n3.rel, t2(n3);
        }
      },
      __nextHasNoMarginBottom: true
    }), React.createElement(wp.components.Button, {
  ...
  s({
    ...
    loader: () => r(() => import("../../html-B3FgNJkv.js").then(async (m2) => {
      await m2.__tla;
      return m2;
    }), __vite__mapDeps([0]), import.meta.url)
  });
  var v = `` + new URL(`../../html.worker-BO6WuOEO.js`, import.meta.url).href;
  ...
  function y({ value: e2 = ``, onChange: t2, language: r2 = `html`, height: i2 = 400, options: a2 = {} }) {
    let s2 = wp.element.useRef(null), c2 = wp.element.useRef(null), l2 = wp.element.useRef(null), u2 = wp.element.useRef(e2), d2 = wp.element.useRef(false), [f2, p2] = wp.element.useState(() => typeof document < `u` ? document.documentElement.classList.contains(`is-dark-theme`) || document.body.classList.contains(`is-dark-theme`) : false), m2 = wp.element.useMemo(() => ({
      ...
      a2
    ]), h2 = wp.element.useMemo(() => t2 ? n((e3) => {
      d2.current = true, t2(e3);
    }, 500) : null, [
      t2
    ...
    return wp.element.useEffect(() => () => {
      ...
      t3.observe(document.documentElement, {
        attributes: true,
        attributeFilter: [
          `class`
        ]
      }), t3.observe(document.body, {
        attributes: true,
        attributeFilter: [
          `class`
        ]
      });
      let n2 = o.create(c2.current, {
        value: e2,
        language: r2,
        theme: f2 ? `vs-dark` : `vs`,
        ...m2
      });
      return s2.current = n2, u2.current = e2, l2.current = n2.onDidChangeModelContent(() => {
        g2(n2.getValue());
      }), () => {
        t3.disconnect(), l2.current && (l2.current = (l2.current.dispose(), null)), s2.current && (s2.current = (s2.current.dispose(), null));
      };
    }, []), wp.element.useEffect(() => {
      ...
      if (e2 !== u2.current && s2.current) {
        let t3 = s2.current;
        if (e2 !== t3.getValue()) {
          let n2 = t3.getPosition(), r3 = t3.getSelection();
          t3.setValue(e2), u2.current = e2, n2 && t3.setPosition(n2), r3 && t3.setSelection(r3), t3.focus();
        }
      }
    }, [
      e2
  ...
  function T(e2, n2) {
    return e2.clientId === n2.clientId ? t(e2.attributes, n2.attributes) : false;
  }
