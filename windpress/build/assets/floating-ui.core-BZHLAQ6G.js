...
function D(e2) {
  return { top: e2.top ?? 0, right: e2.right ?? 0, bottom: e2.bottom ?? 0, left: e2.left ?? 0 };
}
function O(e2) {
...
function A(e2, t2, n2) {
  ...
  let y2 = f(t2);
  return y2 && (v2[o2] += _2 * (y2 === `end` ? 1 : -1) * (n2 && l2 ? -1 : 1)), v2;
}
...
var M = 50, N = async (e2, t2, n2) => {
  ...
  let j2 = E2 / 2 - D2 / 2, M2 = A2 / 2 - x2[b2] / 2 - 1, N2 = r(_2[C2], M2), P2 = r(_2[w2], M2), F2 = A2 - x2[b2] - P2, I2 = A2 / 2 - x2[b2] / 2 + j2, L2 = l(N2, I2, F2), R2 = !d2.arrow && f(a2) != null && I2 !== L2 && o2.reference[b2] / 2 - (I2 < N2 ? N2 : P2) - x2[b2] / 2 < 0, z2 = R2 ? I2 < N2 ? I2 - N2 : I2 - F2 : 0;
  return { [y2]: v2[y2] + z2, data: { [y2]: L2, centerOffset: I2 - L2 - z2, ...R2 && { alignmentOffset: z2 } }, reset: R2 };
} });
function F(e2, t2, n2) {
  return (e2 ? [...n2.filter((t3) => f(t3) === e2), ...n2.filter((t3) => f(t3) !== e2)] : n2.filter((e3) => d(e3) === e3)).filter((n3) => !e2 || f(n3) === e2 || (t2 ? y(n3) !== n3 : false));
}
var I = function(e2) {
  return e2 === void 0 && (e2 = {}), { name: `autoPlacement`, options: e2, async fn(t2) {
    let { rects: r2, middlewareData: i2, placement: a2, platform: o2, elements: s2 } = t2, { crossAxis: c2 = false, alignment: l2, allowedPlacements: p2 = n, autoAlignment: m2 = true, ...h2 } = u(e2, t2), g2 = l2 !== void 0 || p2 === n ? F(l2 || null, m2, p2) : p2, v2 = i2.autoPlacement?.index || 0, y2 = g2[v2];
    if (y2 == null) return {};
    if (a2 !== y2) return { reset: { placement: g2[0] } };
    let b2 = await o2.detectOverflow(t2, h2), x2 = _(y2, r2, await (o2.isRTL == null ? void 0 : o2.isRTL(s2.floating))), S2 = [b2[d(y2)], b2[x2[0]], b2[x2[1]]], C2 = [...i2.autoPlacement?.overflows || [], { placement: y2, overflows: S2 }], w2 = g2[v2 + 1];
    if (w2) return { data: { index: v2 + 1, overflows: C2 }, reset: { placement: w2 } };
    let T2 = C2.map((e3) => {
    ...
    return E2 === a2 ? {} : { data: { index: v2 + 1, overflows: C2 }, reset: { placement: E2 } };
  } };
}, L = function(e2) {
  return e2 === void 0 && (e2 = {}), { name: `flip`, options: e2, async fn(t2) {
    ...
    if (M2 = [...M2, { placement: r2, overflows: j2 }], !j2.every((e3) => e3 <= 0)) {
      ...
      if (t3 && (!(f2 === `alignment` && S2 !== h(t3)) || M2.every((e4) => h(e4.placement) !== S2 || e4.overflows[0] > 0))) return { data: { index: e3, overflows: M2 }, reset: { placement: t3 } };
...
var U = function(e2) {
  ...
  return e2 === void 0 && (e2 = {}), { name: `shift`, options: e2, async fn(t2) {
    let { x: n2, y: r2, placement: i2, platform: a2 } = t2, { mainAxis: o2 = true, crossAxis: s2 = false, limiter: c2 = { fn: (e3) => {
      let { x: t3, y: n3 } = e3;
      return { x: t3, y: n3 };
    } }, ...d2 } = u(e2, t2), f2 = { x: n2, y: r2 }, m2 = await a2.detectOverflow(t2, d2), g2 = h(i2), _2 = p(g2), v2 = f2[_2], y2 = f2[g2], b2 = (e3, t3) => l(t3 + m2[e3 === `y` ? `top` : `left`], t3, t3 - m2[e3 === `y` ? `bottom` : `right`]);
    o2 && (v2 = b2(_2, v2)), s2 && (y2 = b2(g2, y2));
    let x2 = c2.fn({ ...t2, [_2]: v2, [g2]: y2 });
    return { ...x2, data: { x: x2.x - n2, y: x2.y - r2, enabled: { [_2]: o2, [g2]: s2 } } };
  } };
}, G = function(e2) {
  return e2 === void 0 && (e2 = {}), { options: e2, fn(t2) {
    let { x: n2, y: r2, placement: i2, rects: a2, middlewareData: o2 } = t2, { offset: s2 = 0, mainAxis: c2 = true, crossAxis: l2 = true } = u(e2, t2), f2 = { x: n2, y: r2 }, m2 = h(i2), g2 = p(m2), _2 = f2[g2], v2 = f2[m2], y2 = u(s2, t2), b2 = typeof y2 == `number` ? { mainAxis: y2, crossAxis: 0 } : { mainAxis: y2.mainAxis ?? 0, crossAxis: y2.crossAxis ?? 0 };
    if (c2) {
  ...
  return e2 === void 0 && (e2 = {}), { name: `size`, options: e2, async fn(t2) {
    let { placement: n2, rects: a2, platform: o2, elements: s2 } = t2, { apply: c2 = () => {
    }, ...l2 } = u(e2, t2), p2 = await o2.detectOverflow(t2, l2), m2 = d(n2), g2 = f(n2), _2 = h(n2) === `y`, { width: v2, height: y2 } = a2.floating, b2, x2;
    m2 === `top` || m2 === `bottom` ? (b2 = m2, x2 = g2 === (await (o2.isRTL == null ? void 0 : o2.isRTL(s2.floating)) ? `start` : `end`) ? `left` : `right`) : (x2 = m2, b2 = g2 === `end` ? `top` : `bottom`);
    let S2 = y2 - p2.top - p2.bottom, C2 = v2 - p2.left - p2.right, w2 = r(y2 - p2[b2], S2), T2 = r(v2 - p2[x2], C2), E2 = t2.middlewareData.shift, D2 = !E2, O2 = w2, k2 = T2;
    E2 != null && E2.enabled.x && (k2 = C2), E2 != null && E2.enabled.y && (O2 = S2), D2 && !g2 && (_2 ? k2 = v2 - 2 * i(p2.left, p2.right) : O2 = y2 - 2 * i(p2.top, p2.bottom)), await c2({ ...t2, availableWidth: k2, availableHeight: O2 });
    let A2 = await o2.getDimensions(s2.floating);
    return v2 !== A2.width || y2 !== A2.height ? { reset: { rects: true } } : {};
  } };
};
export {
  B as a,
