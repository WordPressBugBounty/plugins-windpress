import { a as e, o as t, t as n } from "./chunk-DlkHe4MQ.js";
import { n as r, t as i } from "./dist-DkW5GlYv.js";
import { t as a } from "./preload-helper-Cb81DA_d.js";
import { n as o } from "./base64-DYxoxXqt.js";
import { a as s, i as c, s as l, t as u } from "./lib-SBX7jFNp.js";
import { t as d } from "./path-browserify-CWnXpqR1.js";
let L, g, R, m, J, w, K, v, Y, f;
let __tla = (async () => {
  f = function(e2) {
    try {
      let t2 = new URL(e2);
  ...
  g = async function(e2, t2 = `/`, n2, r2 = {}) {
    t2 ?? (t2 = `/`);
    let i2 = p.default.resolve(t2, e2);
    if (!r2[i2]) throw Error(`The ${n2} file "${p.default.resolve(`/`, e2)}" does not exist in the volume.`);
...
`), i2 = /import\s*(?:[^'"]*\s*from\s*)?['"]([^'"]+)['"]|import\(\s*['"]([^'"]+)['"]\s*\)/g, a2 = [], o2, s2 = 0;
    for (; (o2 = i2.exec(r2)) !== null; ) {
      ...
      if (f(s3) || !s3.startsWith(`.`) && !s3.startsWith(`/`)) continue;
      let c2 = p.default.resolve(p.default.dirname(t2), s3);
  ...
  y = n(((e2, t2) => {
    var n2 = v(), r2 = l(), i2 = class e3 extends Error {
...
`,
    ...
    var i2 = class {
      ...
      atrule(e3, t3) {
        let n3 = `@` + e3.name, r3 = e3.params ? this.rawValue(e3, `params`) : ``;
        if (e3.raws.afterName === void 0 ? r3 && (n3 += ` `) : n3 += e3.raws.afterName, e3.nodes) this.block(e3, n3 + r3);
        else {
          let i3 = (e3.raws.between || ``) + (t3 ? `;` : ``);
          this.builder(n3 + r3 + i3, e3);
        }
      }
      beforeAfter(e3, t3) {
        let n3;
...
`)) {
      ...
      block(e3, t3) {
        ...
        this.builder(t3 + n3 + `{`, e3, `start`);
        let r3;
        e3.nodes && e3.nodes.length ? (this.body(e3), r3 = this.raw(e3, `after`)) : r3 = this.raw(e3, `after`, `emptyBody`), r3 && this.builder(r3), this.builder(`}`, e3, `end`);
      }
      body(e3) {
        let t3 = e3.nodes.length - 1;
        for (; t3 > 0 && e3.nodes[t3].type === `comment`; ) --t3;
        let n3 = this.raw(e3, `semicolon`);
        for (let r3 = 0; r3 < e3.nodes.length; r3++) {
          let i3 = e3.nodes[r3], a2 = this.raw(i3, `before`);
          a2 && this.builder(a2), this.stringify(i3, t3 !== r3 || n3);
        }
      }
      comment(e3) {
        let t3 = this.raw(e3, `left`, `commentLeft`), n3 = this.raw(e3, `right`, `commentRight`);
        this.builder(`/*` + t3 + e3.text + n3 + `*/`, e3);
      }
      decl(e3, t3) {
        let n3 = this.raw(e3, `between`, `colon`), r3 = e3.prop + n3 + this.rawValue(e3, `value`);
        e3.important && (r3 += e3.raws.important || ` !important`), t3 && (r3 += `;`), this.builder(r3, e3);
      }
      document(e3) {
        this.body(e3);
      }
      raw(e3, t3, i3) {
        let a2;
        if (i3 || (i3 = t3), t3 && (a2 = e3.raws[t3], a2 !== void 0)) return a2;
        let o2 = e3.parent;
        if (i3 === `before` && (!o2 || o2.type === `root` && o2.first === e3 || o2 && o2.type === `document`)) return ``;
        if (!o2) return n2[i3];
        let s2 = e3.root();
        if (s2.rawCache || (s2.rawCache = {}), s2.rawCache[i3] !== void 0) return s2.rawCache[i3];
        if (i3 === `before` || i3 === `after`) return this.beforeAfter(e3, i3);
        {
          let n3 = `raw` + r2(i3);
          this[n3] ? a2 = this[n3](s2, e3) : s2.walk((e4) => {
            if (a2 = e4.raws[t3], a2 !== void 0) return false;
          });
        }
...
` ? (t3 = 1, n3 += 1) : t3 += 1;
  ...
  A = n(((e2, t2) => {
    r();
    var { existsSync: n2, readFileSync: a2 } = u(), { dirname: o2, join: s2 } = d(), { SourceMapConsumer: c2, SourceMapGenerator: f2 } = l();
    function p2(e3) {
    ...
    var m2 = class {
      ...
      consumer() {
        return this.consumerCache || (this.consumerCache = new c2(this.text)), this.consumerCache;
      }
      decodeInline(e3) {
        ...
        let a3 = e3.match(/data:application\/json;([^,]+),/)[1];
        throw Error(`Unsupported source map encoding ` + a3);
      }
      getAnnotationURL(e3) {
      ...
      loadMap(e3, t3) {
        ...
        if (t3) {
          ...
          if (typeof t3 == `function`) {
            let n3 = t3(e3);
            if (n3) {
              let e4 = this.loadFile(n3);
              if (!e4) throw Error(`Unable to load previous source map: ` + n3.toString());
              return e4;
            }
          } else if (t3 instanceof c2) return f2.fromSourceMap(t3).toString();
          ...
          return e3 && (t4 = s2(o2(e3), t4)), this.loadFile(t4);
        }
      }
      startWith(e3, t3) {
  ...
  j = n(((t2, n2) => {
    var { nanoid: r2 } = k(), { isAbsolute: i2, resolve: a2 } = d(), { SourceMapConsumer: o2, SourceMapGenerator: u2 } = l(), { fileURLToPath: f2, pathToFileURL: p2 } = (c(), e(s)), m2 = y(), h2 = A(), g2 = l(), _2 = /* @__PURE__ */ Symbol(`lineToIndexCache`), v2 = !!(o2 && u2), b2 = !!(a2 && i2);
    function x2(e2) {
...
`,
  ...
  I = n(((t2, n2) => {
    r();
    var { dirname: a2, relative: o2, resolve: u2, sep: f2 } = d(), { SourceMapConsumer: p2, SourceMapGenerator: m2 } = l(), { pathToFileURL: h2 } = (c(), e(s)), g2 = j(), _2 = !!(p2 && m2), v2 = !!(a2 && u2 && o2 && f2);
    n2.exports = class {
...
`), t3 = o3.length - i2) : t3 += o3.length, s2 && c2 !== `start`) {
  ...
  L = n(((e2, t2) => {
    ...
    t2.exports = function(e3, t3 = {}) {
      let T2 = e3.css.valueOf(), E2 = t3.ignoreErrors, D2, O2, k2, A2, j2, M2, N2, P2, F2, I2, L2 = T2.length, R2 = 0, z2 = [], B2 = [];
      function V2() {
        return R2;
      }
      function H2(t4) {
  ...
  G = n(((e2, t2) => {
    var n2 = O(), r2 = U(), i2 = W(), a2 = M(), o2 = class {
      constructor(e3 = []) {
        this.version = `8.5.8`, this.plugins = this.normalize(e3);
      }
      normalize(e3) {
        let t3 = [];
