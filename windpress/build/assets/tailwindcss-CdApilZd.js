...
import { t as e } from "./log-Dd9BHdsz.js";
import { t } from "./api-DL2Um68W.js";
import { t as n } from "./preload-helper-CtT0-S4g.js";
import { n as r, t as i } from "./dist-u5p_KAIj.js";
import { __tla as __tla_0 } from "./cssesc-BusMg9uA.js";
import "./build-DOFDHjo8.js";
import "./vfs-DGZNrgMJ.js";
import "./intellisense-BqSPmEm7.js";
let l, c, Je;
let __tla = Promise.all([
  ...
  c = async function(e2) {
    if (!o) return s || (s = (async () => {
      try {
        let t2 = await n(() => import("./oxide_parser-CzkuMXQD-DNzqorn0.js").then(async (m2) => {
          await m2.__tla;
          return m2;
        }), [], import.meta.url);
  ...
  function C(e2, t2 = {}) {
    if (!e2) return [];
    let { max: n2 = se } = t2;
    return e2.slice(0, 2) === `{}` && (e2 = `\\{\\}` + e2.slice(2)), E(ce(e2), n2, true).map(le);
  }
  function w(e2) {
    return `{` + e2 + `}`;
  }
  ...
  function E(e2, t2, n2) {
    ...
    for (; ; ) {
      ...
      let a2 = i2.pre;
      if (/\$$/.test(i2.pre)) {
        let e3 = i2.post.length ? E(i2.post, t2, false) : [
          ``
        ];
        for (let n3 = 0; n3 < e3.length && n3 < t2; n3++) {
          let t3 = a2 + `{` + i2.body + `}` + e3[n3];
          r2.push(t3);
        }
        return r2;
      }
      let o2 = /^-?\d+\.\.-?\d+(?:\.\.-?\d+)?$/.test(i2.body), s2 = /^[a-zA-Z]\.\.[a-zA-Z](?:\.\.-?\d+)?$/.test(i2.body), c2 = o2 || s2, l2 = i2.body.indexOf(`,`) >= 0;
      if (!c2 && !l2) {
        if (i2.post.match(/,(?!,).*\}/)) {
          e2 = i2.pre + `{` + i2.body + h + i2.post, n2 = true;
          continue;
        }
        return [
          e2
      ...
      let d2 = i2.post.length ? E(i2.post, t2, false) : [
        ``
      ], f2;
      if (c2) f2 = i2.body.split(/\.\./);
      else if (f2 = S(i2.body), f2.length === 1 && f2[0] !== void 0 && (f2 = E(f2[0], t2, false).map(w), f2.length === 1)) return d2.map((e3) => i2.pre + f2[0] + e3);
      let p2;
      if (c2 && f2[0] !== void 0 && f2[1] !== void 0) {
        let e3 = x(f2[0]), n3 = x(f2[1]), r3 = Math.max(f2[0].length, f2[1].length), i3 = f2.length === 3 && f2[2] !== void 0 ? Math.max(Math.abs(x(f2[2])), 1) : 1, a3 = ue;
        n3 < e3 && (i3 *= -1, a3 = de);
        let o3 = f2.some(T);
        p2 = [];
        for (let c3 = e3; a3(c3, n3) && p2.length < t2; c3 += i3) {
          let e4;
          if (s2) e4 = String.fromCharCode(c3), e4 === `\\` && (e4 = ``);
          else if (e4 = String(c3), o3) {
            let t3 = r3 - e4.length;
            if (t3 > 0) {
              let n4 = Array(t3 + 1).join(`0`);
              e4 = c3 < 0 ? `-` + n4 + e4.slice(1) : n4 + e4;
            }
          }
          p2.push(e4);
        }
        ...
        p2 = [];
        for (let e3 = 0; e3 < f2.length; e3++) p2.push.apply(p2, E(f2[e3], t2, false));
      }
      for (let e3 = 0; e3 < p2.length; e3++) for (let i3 = 0; i3 < d2.length && r2.length < t2; i3++) {
        let t3 = a2 + p2[e3] + d2[i3];
        (!n2 || c2 || t3) && r2.push(t3);
      }
      return r2;
    }
  ...
  var fe = 1024 * 64, D = (e2) => {
    ...
    if (e2.length > fe) throw TypeError(`pattern is too long`);
  }, pe = {
    "[:alnum:]": [
      `\\p{L}\\p{Nl}\\p{Nd}`,
      true
  ...
  }, O = (e2) => e2.replace(/[[\]\\-]/g, `\\$&`), me = (e2) => e2.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, `\\$&`), k = (e2) => e2.join(``), A = (e2, t2) => {
    let n2 = t2;
    ...
    WHILE: for (; a2 < e2.length; ) {
      ...
      if (t3 === `[` && !c2) {
        for (let [t4, [o3, c3, l3]] of Object.entries(pe)) if (e2.startsWith(t4, a2)) {
          if (d2) return [
            `$.`,
            false,
            e2.length - n2,
      ...
      if (c2 = false, d2) {
        t3 > d2 ? r2.push(O(d2) + `-` + O(t3)) : t3 === d2 && r2.push(O(t3)), d2 = ``, a2++;
        continue;
      }
    ...
    if (i2.length === 0 && r2.length === 1 && /^\\?.$/.test(r2[0]) && !l2) return [
      me(r2[0].length === 2 ? r2[0].slice(-1) : r2[0]),
      false,
      u2 - n2,
      false
    ];
    let f2 = `[` + (l2 ? `^` : ``) + k(r2) + `]`, p2 = `[` + (l2 ? `` : `^`) + k(i2) + `]`;
    return [
  ...
  }, j = (e2, { windowsPathsNoEscape: t2 = false, magicalBraces: n2 = true } = {}) => n2 ? t2 ? e2.replace(/\[([^/\\])\]/g, `$1`) : e2.replace(/((?!\\).|^)\[([^/\\])\]/g, `$1$2`).replace(/\\([^/])/g, `$1`) : t2 ? e2.replace(/\[([^/\\{}])\]/g, `$1`) : e2.replace(/((?!\\).|^)\[([^/\\{}])\]/g, `$1$2`).replace(/\\([^/{}])/g, `$1`), M, N = /* @__PURE__ */ new Set([
    `!`,
    `?`,
    `+`,
    `*`,
    `@`
  ]), P = (e2) => N.has(e2), F = (e2) => P(e2.type), I = /* @__PURE__ */ new Map([
    [
      `!`,
      [
        `@`
  ...
  ]), he = `(?!(?:^|/)\\.\\.?(?:$|/))`, B = `(?!\\.)`, ge = /* @__PURE__ */ new Set([
    `[`,
    `.`
  ]), _e = /* @__PURE__ */ new Set([
    `..`,
    `.`
  ]), ve = new Set(`().*{}+?[]^$\\!`), ye = (e2) => e2.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, `\\$&`), be = `[^/]`, xe = `[^/]*?`, V = `[^/]+?`, Se = 0, H = (_a2 = class {
    ...
    push(...e2) {
      for (let t2 of e2) if (t2 !== ``) {
        if (typeof t2 != `string` && !(t2 instanceof M && __privateGet(t2, _i) === this)) throw Error(`invalid part: ` + t2);
    ...
    isStart() {
      ...
      for (let t2 = 0; t2 < __privateGet(this, _a); t2++) {
        ...
        if (!(n2 instanceof M && n2.type === `!`)) return false;
      }
      return true;
    }
    isEnd() {
    ...
    clone(e2) {
      let t2 = new M(this.type, e2);
      for (let e3 of __privateGet(this, _r)) t2.copyIn(e3);
      return t2;
    }
    static fromGlob(e2, t2 = {}) {
      var _a3;
      let n2 = new M(null, void 0, t2);
      return __privateMethod(_a3 = M, _H_static, f_fn).call(_a3, e2, n2, 0, t2, 0), n2;
    }
    toMMPattern() {
    ...
    toRegExpSource(e2) {
      ...
      if (__privateGet(this, _e2) === this && (__privateMethod(this, _H_instances, x_fn).call(this), __privateMethod(this, _H_instances, d_fn).call(this)), !F(this)) {
        let n3 = this.isStart() && this.isEnd() && !__privateGet(this, _r).some((e3) => typeof e3 != `string`), r3 = __privateGet(this, _r).map((t3) => {
          var _a3;
          let [r4, i4, a4, o3] = typeof t3 == `string` ? __privateMethod(_a3 = M, _H_static, C_fn).call(_a3, t3, __privateGet(this, _t), n3) : t3.toRegExpSource(e2);
          return __privateSet(this, _t, __privateGet(this, _t) || a4), __privateSet(this, _n, __privateGet(this, _n) || o3), r4;
        }).join(``), i3 = ``;
        if (this.isStart() && typeof __privateGet(this, _r)[0] == `string` && !(__privateGet(this, _r).length === 1 && _e.has(__privateGet(this, _r)[0]))) {
          let n4 = ge, a4 = t2 && n4.has(r3.charAt(0)) || r3.startsWith(`\\.`) && n4.has(r3.charAt(2)) || r3.startsWith(`\\.\\.`) && n4.has(r3.charAt(4)), o3 = !t2 && !e2 && n4.has(r3.charAt(0));
          i3 = a4 ? he : o3 ? B : ``;
        }
        let a3 = ``;
      ...
      let o2 = ``;
      if (this.type === `!` && __privateGet(this, _u)) o2 = (this.isStart() && !t2 ? B : ``) + V;
      else {
        let n3 = this.type === `!` ? `))` + (this.isStart() && !t2 && !e2 ? B : ``) + `[^/]*?)` : this.type === `@` ? `)` : this.type === `?` ? `)?` : this.type === `+` && a2 ? `)` : this.type === `*` && a2 ? `)?` : `)${this.type}`;
        o2 = r2 + i2 + n3;
      }
      return [
        o2,
    ...
    if (t2.type === null) {
      ...
      for (; u3 < e2.length; ) {
        ...
        if (!r2.noext && P(n3) && e2.charAt(u3) === `(` && i2 <= a2) {
          t2.push(d3), d3 = ``;
          let a3 = new M(n3, t2);
          u3 = __privateMethod(_a3 = M, _H_static, f_fn).call(_a3, e2, a3, u3, r2, i2 + 1), t2.push(a3);
          continue;
        }
        d3 += n3;
      }
    ...
    let u2 = n2 + 1, d2 = new M(null, t2), f2 = [], p2 = ``;
    for (; u2 < e2.length; ) {
      let n3 = e2.charAt(u2++);
      if (o2 || n3 === `\\`) {
        o2 = !o2, p2 += n3;
      ...
      if (!r2.noext && P(n3) && e2.charAt(u2) === `(` && (i2 <= a2 || t2 && __privateMethod(_b2 = t2, _H_instances, h_fn).call(_b2, n3))) {
        let a3 = t2 && __privateMethod(_c2 = t2, _H_instances, h_fn).call(_c2, n3) ? 0 : 1;
        d2.push(p2), p2 = ``;
        let o3 = new M(n3, d2);
        d2.push(o3), u2 = __privateMethod(_d = M, _H_static, f_fn).call(_d, e2, o3, u2, r2, i2 + a3);
        continue;
      }
      if (n3 === `|`) {
        d2.push(p2), p2 = ``, f2.push(d2), d2 = new M(null, t2);
        continue;
      }
    ...
    return __privateMethod(this, _H_instances, m_fn).call(this, e2, L);
  }, m_fn = function(e2, t2 = I) {
  ...
  }, h_fn = function(e2, t2 = R) {
    return !!t2.get(this.type)?.includes(e2);
  }, g_fn = function(e2, t2) {
    let n2 = __privateGet(e2, _r)[0], r2 = new M(null, n2, this.options);
    ...
    let t2 = z.get(this.type), n2 = __privateGet(e2, _r)[0], r2 = t2?.get(n2.type);
    if (!r2) return false;
    ...
    for (let s2 = 0; s2 < e2.length; s2++) {
      let c2 = e2.charAt(s2);
      if (r2) {
        r2 = false, i2 += (ve.has(c2) ? `\\` : ``) + c2;
        continue;
      }
      if (c2 === `*`) {
        if (o2) continue;
        o2 = true, i2 += n2 && /^[*]+$/.test(e2) ? V : xe, t2 = true;
        continue;
      } else o2 = false;
      if (c2 === `\\`) {
  ...
  M = H;
  var Ce = (e2, { windowsPathsNoEscape: t2 = false, magicalBraces: n2 = false } = {}) => n2 ? t2 ? e2.replace(/[?*()[\]{}]/g, `[$&]`) : e2.replace(/[?*()[\]\\{}]/g, `\\$&`) : t2 ? e2.replace(/[?*()[\]]/g, `[$&]`) : e2.replace(/[?*()[\]\\]/g, `\\$&`), U = (e2, t2, n2 = {}) => (D(t2), !n2.nocomment && t2.charAt(0) === `#` ? false : new $(t2, n2).match(e2)), we = /^\*+([^+@!?*[(]*)$/, Te = (e2) => (t2) => !t2.startsWith(`.`) && t2.endsWith(e2), Ee = (e2) => (t2) => t2.endsWith(e2), W = (e2) => (e2 = e2.toLowerCase(), (t2) => !t2.startsWith(`.`) && t2.toLowerCase().endsWith(e2)), De = (e2) => (e2 = e2.toLowerCase(), (t2) => t2.toLowerCase().endsWith(e2)), Oe = /^\*+\.\*+$/, ke = (e2) => !e2.startsWith(`.`) && e2.includes(`.`), Ae = (e2) => e2 !== `.` && e2 !== `..` && e2.includes(`.`), je = /^\.\*+$/, Me = (e2) => e2 !== `.` && e2 !== `..` && e2.startsWith(`.`), Ne = /^\*+$/, Pe = (e2) => e2.length !== 0 && !e2.startsWith(`.`), Fe = (e2) => e2.length !== 0 && e2 !== `.` && e2 !== `..`, Ie = /^\?+([^+@!?*[(]*)?$/, Le = ([e2, t2 = ``]) => {
    let n2 = G([
      e2
    ]);
  ...
  var Z = (e2, t2 = {}) => (D(e2), t2.nobrace || !/\{(?:(?!\{).)*\}/.test(e2) ? [
    e2
  ] : C(e2, {
  ...
  var Q = /[?*]|[+@!]\(.*?\)|\[|\]/, We = (e2) => e2.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, `\\$&`), $ = (_b = class {
    ...
    parse(e2) {
      ...
      if (e2 === ``) return ``;
      let n2, r2 = null;
      (n2 = e2.match(Ne)) ? r2 = t2.dot ? Fe : Pe : (n2 = e2.match(we)) ? r2 = (t2.nocase ? t2.dot ? De : W : t2.dot ? Ee : Te)(n2[1]) : (n2 = e2.match(Ie)) ? r2 = (t2.nocase ? t2.dot ? Re : Le : t2.dot ? ze : Be)(n2) : (n2 = e2.match(Oe)) ? r2 = t2.dot ? Ae : ke : (n2 = e2.match(je)) && (r2 = Me);
  ...
  U.AST = H, U.Minimatch = $, U.escape = Ce, U.unescape = j;
  var Ge = /^(@[^\/]+\/[^@\/]+)(?:@([^\/]+))?(\/.*)?$/, Ke = /^([^@\/]+)(?:@([^\/]+))?(\/.*)?$/;
  function qe(e2) {
