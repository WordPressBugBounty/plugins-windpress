...
(function() {
  ...
  window.fetch = async (...n) => {
    ...
    if (new URL(n[0]).searchParams.get(`_breakdance_doing_ajax`) === `yes`) {
      ...
      r.ok && r.status === 200 && e3.action === `breakdance_save` && t.postMessage({ task: `generate-cache`, source: `windpress/integration`, target: `windpress/compiler`, data: { kind: `incremental`, incremental: { providers: [`oxygen`] } } });
    }
    return r;
  };
